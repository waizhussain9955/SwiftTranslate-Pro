...
// BYPASS 402: Fetch content locally instead of using GTranslate proxy
...
$page_url = $protocol . '://' . $page_host . $page_url;
...
if($response === false) {
    $error = curl_error($ch);
    curl_close($ch);
    echo 'GTranslate Proxy Error: ' . $error . '. Please check loopback connections on your server.';
    exit;
}
...
// Ensure static assets and system paths are NOT prefixed (SEO Standard)
$html = str_ireplace('href="/' . $glang . '/wp-content/', 'href="/wp-content/', $html);
$html = str_ireplace('href="/' . $glang . '/wp-includes/', 'href="/wp-includes/', $html);
$html = str_ireplace('src="/' . $glang . '/', 'src="/', $html);
$html = str_ireplace('data-src="/' . $glang . '/', 'data-src="/', $html);
$html = str_ireplace('srcset="/' . $glang . '/', 'srcset="/', $html);

// INJECT TRANSLATION TRIGGER & REDIRECT LOGIC
$trigger_script = '<style>
    /* Aggressively hide Google Translate Toolbar */
    .goog-te-banner-frame, .goog-te-banner, #goog-gt-tt, .goog-te-balloon-frame, #goog-gt-t { display: none !important; visibility: hidden !important; }
    body { top: 0 !important; }
    .skiptranslate { display: none !important; }
    #google_translate_element2 { display:none!important; }
    
    /* Ensure widget stays on top and visible */
    .gtranslate_wrapper { 
...
<script>
    window.gtranslateSettings = {
        ...
        "languages": ["en","es","it","pt","de","fr","ru","nl","ar","zh-CN","ur","bn","hi","tr","id"],
        "url_structure": "sub_directory",
        "flag_size": 24,
        "alt_flags": {"en":"usa", "ur":"pakistan"},
        "wrapper_selector": ".gtranslate_wrapper"
    };

    // Emergency check for wrapper
    ...
    if ("'.$glang.'" != "'.$main_lang.'") {
        ...
        document.cookie = "googtrans=/'.$main_lang.'/'.$glang.'; path=/; domain=.'.$_SERVER['HTTP_HOST'].'";
    }

    function googleTranslateElementInit2() {
        new google.translate.TranslateElement({pageLanguage: "'.$main_lang.'", autoDisplay: false}, "google_translate_element2");
    }
</script>
