...
class GTranslate extends WP_Widget {
    ...
    public static function activate() {
        ...
        $data['gtranslate_title'] = isset($data['gtranslate_title']) ? $data['gtranslate_title'] : 'Website Translator';
        self::load_defaults($data);

        // Save options first
        update_option('GTranslate', $data);

        // Auto-update .htaccess with URL rewrite rules
        self::update_htaccess($data);

        // Auto-update config.php with correct main language
        self::update_config_php($data);
    }

    public static function update_htaccess($data) {
        ...
        // Remove old GTranslate rules first (clean slate)
        $htaccess = preg_replace('/### BEGIN GTranslate config ###.*?### END GTranslate config ###\r?\n?/s', '', $htaccess);

        // Build new rewrite rules with correct path
        $plugin_relative_path = str_replace(
            str_replace(array('https:', 'http:'), array('', ''), home_url()),
            '',
            str_replace(array('https:', 'http:'), array('', ''), plugins_url())
        ) . '/gtranslate';

        $rewrite_rules = file_get_contents(dirname(__FILE__) . '/url_addon/rewrite.txt');
        $rewrite_rules = str_replace('GTRANSLATE_PLUGIN_PATH', $plugin_relative_path, $rewrite_rules);

        // Prepend rules before WordPress rules
    ...
    public static function update_config_php($data) {
...
?>
    ...
    public static function control_options() {
        ...
        $data['pro_version'] = '1'; // URL addon unlocked
        ...
        $data['url_translation'] = '1'; // auto-enabled: full website URL translation
        $data['add_hreflang_tags'] = '1'; // auto-enabled: hreflang SEO tags for all languages
        ...
        // Always re-apply htaccess rules and sync config.php on save
        self::update_htaccess($data);
        self::update_config_php($data);
    }
    ...
    public static function load_settings($data) {
        ...
        $gt_settings['url_structure'] = 'sub_directory'; // Force URL redirect on language switch
    ...
    public static function load_defaults(& $data) {
        ...
        $data['pro_version'] = '1'; // URL addon unlocked - sub_directory URL structure enabled
        $data['enterprise_version'] = ''; // Turn off enterprise server check
        ...
        $data['show_in_menu'] = isset($data['show_in_menu']) ? $data['show_in_menu'] : '';

        // Let user choose their widget look and position in Settings
        ...
        $data['detect_browser_language'] = '1';
...
if($data['add_hreflang_tags'] and ($data['pro_version'] or $data['enterprise_version'])) {
    function gtranslate_add_hreflang_tags() {
        ...
        foreach($enabled_languages as $lang) {
            ...
            // Force Sub-directory structure for SEO URLs
